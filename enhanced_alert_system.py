...
def show_enhanced_gui_alert(accident_lat: float = None, accident_lon: float = None,
    ...
    def on_view_route():
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to generate route: {str(e)}")
            traceback.print_exc()
    ...
    # Main label
    tk.Label(root, text="🚨 ACCIDENT DETECTED! 🚨", 
             font=("Arial", 18, "bold"), fg="red").pack(pady=10)
    
    tk.Label(root, text=f"Location: {accident_location_name}", 
             font=("Arial", 12)).pack(pady=5)
    
    if accident_lat and accident_lon:
        tk.Label(root, text=f"Coordinates: ({accident_lat:.6f}, {accident_lon:.6f})", 
    ...
    try:
        ...
        if os.path.exists(gif_path):
            ...
            tk.Label(root, text="⚠️ Alert animation", fg="orange", 
    ...
    if accident_lat and accident_lon:
        tk.Label(info_frame, text="Emergency Response Options:", 
    ...
    if accident_lat and accident_lon:
        tk.Button(button_frame, text="🗺️ View Route Map", 
                 command=on_view_route, bg="#2196f3", fg="white", 
                 font=("Arial", 12, "bold"), padx=20, pady=10).pack(side=tk.LEFT, padx=10)
    
    tk.Button(button_frame, text="📞 Make Emergency Call", 
             command=on_make_call, bg="red", fg="white", 
             font=("Arial", 12, "bold"), padx=20, pady=10).pack(side=tk.LEFT, padx=10)
...
def show_gui_alert(message="⚠️ Accident Detected!", 
                  accident_lat: float = None, 
                  accident_lon: float = None):
    """
    Backward-compatible wrapper for existing alert system
    Falls back to enhanced alert if coordinates provided
    """
...
if __name__ == "__main__":
    ...
    show_enhanced_gui_alert(
        accident_lat=13.074,
        accident_lon=80.24,
        accident_location_name="Chennai - Test Location"
    )
